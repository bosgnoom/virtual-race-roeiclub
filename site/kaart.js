...
function loadWayPoints(routenummer) {
  ...
  if (routenummer == -1) {
    ...
    routeItems.forEach(function (i) {
      var waypoint = virtualrace.routes.values[routenummer][i].toString();

      if (waypoint.indexOf(";") < 0) {
        // only one waypoint. Is going to be the old method
        ...
        // ; found --> multiple points --> polygon
        // Most likely this can be optimized...
        locations = [];
        points = waypoint.split(";");
        for (let point of points) {
          console.log("Waypoint point:" + point);
    ...
    console.log(
      "Detection radius:" + virtualrace.routes.values[routenummer][11]
    );
...
function setupMarkers(waypoints) {
  ...
  if (Array.isArray(waypoints[0][0])) {
    console.log("Waypoints in array...");
    // TODO: implement *vaag taalgebruik
