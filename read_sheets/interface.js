/*   
	Zet deze in de google sheets, via de script editor. 
...
function retrieve(e) {
	...
	return ContentService.createTextOutput(
		JSON.stringify({ result: "success", values: values })
	).setMimeType(ContentService.MimeType.JSON);
}
...
function update(e) {
	...
	// return json success results
	return ContentService.createTextOutput(
		JSON.stringify({ result: "success", row: rowId })
	).setMimeType(ContentService.MimeType.JSON);
}
...
function findHeader(header, headers) {
	// Oh Christ... Met een for loop? Dan kan beter!
	console.log("findHeader start");
	for (i = 0; i < headers.length; i++) {
		if (headers[i] == header) {
			return i;
		}
	}
	console.log("Hier is het niet OK...");
	return -1;
}
...
function getRaces(e) {
	...
	var kolom = findHeader("Race name", headers);
	console.log("kolom:" + kolom);

	var range = sheet.getRange(2, 1 + kolom, numRows - 1);
